...
namespace BadDetective.UI
{
    public class QuestFile : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (secondFrame)
            {
                float descriptionHeight = Mathf.Abs(shortDescription.rectTransform.offsetMin.y) + 10;
                if (quest.withDeadline)
                {
                    deadline.rectTransform.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top, descriptionHeight, deadline.rectTransform.rect.height);
                    descriptionHeight += deadline.rectTransform.rect.height;
                }
                registrasteToogle.GetComponent<RectTransform>().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top, descriptionHeight, registrasteToogle.GetComponent<RectTransform>().rect.height);
                descriptionHeight += registrasteToogle.GetComponent<RectTransform>().rect.height + 10;
                questObjectivesText.rectTransform.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top, descriptionHeight, questObjectivesText.rectTransform.rect.height);
                descriptionHeight += questObjectivesText.rectTransform.rect.height;
                questObjectivesPanel.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top, descriptionHeight, questObjectivesPanel.rect.height);
                notePages.Clear();
                int counter = 0;
                float h = 0;
