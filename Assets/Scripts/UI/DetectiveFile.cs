...
namespace BadDetective.UI
{
    public class DetectiveFile : MonoBehaviour
    {
        ...
        private void SetDetective()
        {
            ...
            salary.text = string.Format("Salary: {0}", detective.salary.ToString());
           ...
           // experience.text = string.Format("Experience: {0}/{1}/{2}", detective., (int)detective.curLoyalty, detective.maxLoyalty);
            ...
            foreach (Trait trait in detective.traits)
            {
                if (trait.category == TraitCategory.FENOTYPE)
                {
                    fenotype.text = string.Format("Fenotype: {0}", trait.traitName);
                }
